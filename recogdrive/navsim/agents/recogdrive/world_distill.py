...
class StructuralWorldDistillLoss(nn.Module):
    def __init__(
        ...
        self.teacher_proj = nn.ModuleDict({
            "scene": nn.LazyLinear(student_world_dim),
            "agent": nn.LazyLinear(student_world_dim),
            "goal": nn.LazyLinear(student_world_dim),
        })
    ...
    def forward(self, student_world: Dict[str, torch.Tensor], teacher_outputs: Dict[str, Optional[torch.Tensor]]):
        logs = {
            "loss_struct_scene": torch.tensor(0.0, device=next(self.parameters()).device),
            "loss_struct_agent": torch.tensor(0.0, device=next(self.parameters()).device),
            "loss_struct_goal": torch.tensor(0.0, device=next(self.parameters()).device),
            "loss_struct_total": torch.tensor(0.0, device=next(self.parameters()).device),
        }
