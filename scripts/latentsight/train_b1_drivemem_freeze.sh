...
CONFIG=${CONFIG:-${REPO_ROOT}/configs/latentsight/b1_drivemem_freeze_vit_projector.yaml}
STUDENT_CHECKPOINT=${STUDENT_CHECKPOINT:-/home/ma-user/ipfs/tuntun/LSD/premodel/ReCogDrive_Diffusion_Planner_2B_IL.ckpt}
STUDENT_VLM_PATH=${STUDENT_VLM_PATH:-/home/ma-user/ipfs/tuntun/SGDrive-main/ReCogDrive-VLM-2B}
WORK_DIR=${WORK_DIR:-${REPO_ROOT}/exp/latentsight_v2/b1_drivemem_freeze_vit_projector}
...
echo "[LatentSight][Launch] experiment=B1-DriveMem-freezeViT-projector"
echo "[LatentSight][Launch] train_mode=freeze_vit_projector_train_llm_planner"
echo "[LatentSight][Launch] drivemem=true dsu_side_branch=true source=online_hidden_states"
echo "[LatentSight][Launch] teacher=false future_queries=false distillation=false"
...
torchrun \
  ...
  output_dir="${WORK_DIR}" \
  experiment_name=latentsight_v2_b1_drivemem_freeze_vit_projector \
