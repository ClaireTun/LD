...
CONFIG=${CONFIG:-${REPO_ROOT}/configs/latentsight/b3_freeze_vit_dream_distill.yaml}
...
WORK_DIR=${WORK_DIR:-${REPO_ROOT}/exp/latentsight_v4/b3_freeze_vit_dream_distill}
...
echo "[LatentSight][Launch] experiment=B3-freezeViT-dream-distill"
echo "[LatentSight][Launch] train_mode=freeze_vit_projector_train_llm_planner epochs=100 ckpt_every=10 keep=20"
echo "[LatentSight][Launch] teacher=true future_queries=true distillation_keys=[dream_scene,dream_agent] planner_condition=single"
...
torchrun \
  ...
  output_dir="${WORK_DIR}" \
  experiment_name=latentsight_v4_b3_freeze_vit_dream_distill \
